...
export function renderFavoritos() {
  ...
  container.innerHTML = products.map((product, i) => `
    <div class="showcase-item" style="--accent: ${product.accentColor}">
      ${isMobile ? '' : `<div class="item-bg-text">${product.name.replace(/\s+/g, '<br>')}</div>`}
      <div class="item-visual-wrap">
        ...
        ${isMobile ? '' : `<div class="item-reflection"><img src="${product.image}" alt="" class="showcase-img-reflect" loading="lazy" decoding="async" aria-hidden="true"></div>`}
      </div>
      <div class="item-info">
